        ...
        <div className="page-container">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    <GlassCard>
                        <p className="text-white/60 text-sm mb-1">{t('gpsRecords')}</p>
                        <p className="text-3xl font-bold text-white">{stats.total}</p>
                    </GlassCard>
                    <GlassCard>
                        <p className="text-white/60 text-sm mb-1">{t('present')}</p>
                        <p className="text-3xl font-bold text-green-400">{stats.present}</p>
                    </GlassCard>
                    <GlassCard>
                        <p className="text-white/60 text-sm mb-1">{t('late')}</p>
                ...
                {loading ? (
                    <GlassCard><p className="text-white/70 text-center py-8">{tc('loading')}...</p></GlassCard>
                ) : tab === 'pending' ? (
                    /* ── Pending Tab ── */
                    pending.length === 0 ? (
                        <GlassCard>
                            <div className="text-center py-12">
                                ...
                                <p className="text-white font-semibold text-lg">{t('allAccountedFor')}</p>
                                <p className="text-white/60 text-sm mt-1">{t('allAccountedSubtitle')}</p>
                            </div>
                        </GlassCard>
                    ) : (
                        <GlassCard>
                            <div className="mb-4">
                                <h2 className="text-lg font-semibold text-white">{t('missingTitle')}</h2>
                                <p className="text-white/60 text-sm">{t('missingSubtitle')}</p>
                            </div>
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b border-white/10">
                                            <th className="px-4 py-3 text-left text-white/60 font-medium">{t('coachCol')}</th>
                                            <th className="px-4 py-3 text-left text-white/60 font-medium">{t('courseCol')}</th>
                                            <th className="px-4 py-3 text-left text-white/60 font-medium">{t('dateCol')}</th>
                                            <th className="px-4 py-3 text-left text-white/60 font-medium">{t('timeCol')}</th>
                                            <th className="px-4 py-3 text-right text-white/60 font-medium">{t('actionsCol')}</th>
                                        </tr>
                                    </thead>
                                            ...
                                            <tr key={session.id} className="hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="px-4 py-3 text-right">
                                                    <div className="flex justify-end gap-2">
                                                        ...
                                                        >
                                                            {marking === session.id ? '...' : t('markPresent')}
                                                        </button>
                                                        <button
                    ...
                    attendance.length === 0 ? (
                        <GlassCard>
                            <p className="text-white/70 text-center py-8">{t('noRecords')}</p>
                        </GlassCard>
                    ) : (
                        <GlassCard>
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b border-white/10">
                                            ...
                                            <th className="px-4 py-3 text-left text-white/60 font-medium">{t('courseCol')}</th>
                                            <th className="px-4 py-3 text-left text-white/60 font-medium">{t('dateCol')}</th>
                                            <th className="px-4 py-3 text-left text-white/60 font-medium">{t('statusCol')}</th>
                                            <th className="px-4 py-3 text-left text-white/60 font-medium">{t('distanceCol')}</th>
                                            <th className="px-4 py-3 text-left text-white/60 font-medium">{t('sourceCol')}</th>
                                            <th className="px-4 py-3 text-right text-white/60 font-medium">{t('actionsCol')}</th>
                                        </tr>
                                    </thead>
                                            ...
                                            <tr key={record.id} className="hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="px-4 py-3">
                                                    {record.marked_by_admin ? (
                                                        <span className="text-xs px-2 py-0.5 rounded bg-purple-500/20 text-purple-300">{t('adminMarked')}</span>
                                                    ) : (
