                        ...
                        <div>
                            ...
                            {duplicates.length > 0 && (
                                <div className="mt-2 bg-yellow-500/10 border border-yellow-500/30 rounded-lg px-4 py-3">
                                    <p className="text-yellow-400 text-sm font-medium mb-2">âš ï¸ A student with this name already exists:</p>
                                    {duplicates.map((d: any) => (
                                        <div key={d.id} className="text-yellow-300/80 text-xs flex items-center gap-3">
                                            <span>{d.full_name}</span>
                                            {d.phone && <span>ðŸ“ž {d.phone}</span>}
                                            {d.date_of_birth && <span>ðŸŽ‚ {new Date(d.date_of_birth).toLocaleDateString('en-GB')}</span>}
                                        </div>
                                    ))}
