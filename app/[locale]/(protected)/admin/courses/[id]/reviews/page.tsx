...
import { Star, User, Calendar, MessageSquare, ChevronLeft, FileText } from 'lucide-react';
...
export default function AdminCourseReviewsPage() {
    ...
    const fetchData = async () => {
        setLoading(true);
        const [{ data: courseData }, { data: reviewData, error }] = await Promise.all([
        ...
        <div className="page-container">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-white/70">{t('courseReviewsDetail') || 'عرض تفصيلي لجميع تقييمات الكورس'}</p>
                </div>
                ...
                {reviews.length === 0 ? (
                    <GlassCard>
                        <div className="text-center py-16">
                            ...
                            <p className="text-white/40 text-sm">لم يتم استلام أي تقييمات لهذا الكورس بعد</p>
                        </div>
                    </GlassCard>
                ) : (
                                ...
                                <GlassCard key={review.id} delay={index * 0.05}>
                                    ...
                                    {review.profiles?.full_name && (
                                        <div className="flex items-center gap-1 text-sm text-white/50 mb-3">
                                            <User size={14} />
                                            <span>الكوتش: </span>
                                            <span className="text-primary font-medium">{review.profiles.full_name}</span>
                                        </div>
                                    )}
