...
export default function AdminLogActivityPage() {
    ...
    useEffect(() => {
        const load = async () => {
            const { data } = await supabase
                .from('profiles')
                .select('id, full_name, base_hourly_rate, rate_effective_from')
                .eq('role', 'coach')
                .order('full_name');
            setCoaches(data || []);
        };
    ...
    const selectedCoach = coaches.find((c) => c.id === form.coach_id) || null;

    const computedRate =
        selectedCoach?.base_hourly_rate && form.session_date
            ? getBaseRate(
                  Number(selectedCoach.base_hourly_rate),
                  selectedCoach.rate_effective_from || form.session_date,
                  form.session_date
              )
            : null;
            ...
            <div className="max-w-2xl mx-auto">
                ...
                <GlassCard>
                    <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            >
                                ...
                                {coaches.map((c) => (
                                    <option key={c.id} value={c.id} className="bg-gray-900">
                                        {c.full_name}{c.base_hourly_rate ? ` — ${c.base_hourly_rate} EGP/hr base` : ' — no base rate'}
                                    </option>
                                ))}
                            </select>
                        </div>
                        ...
                        {/* Rate preview — only shown when coach is selected */}
                        {form.coach_id && (
                            <div
                            ...
                            >
                                {computedRate ? (
                                    <p className="text-primary font-medium">
                                        💰 Rate for {selectedCoach?.full_name}: <span className="font-bold">{computedRate} EGP/hr</span>
                        ...
                        <button
                            type="submit"
                            disabled={submitting || !computedRate || !form.coach_id}
