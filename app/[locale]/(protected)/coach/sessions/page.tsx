import { createClient } from '@/lib/supabase/server';
import { redirect } from 'next/navigation';
import { GlassCard } from '@/components/layout/GlassCard';
import { formatCurrency, formatDate } from '@/lib/utils';
import Link from 'next/link';
import { getLocale, getTranslations } from 'next-intl/server';
import { DeleteSessionButton } from '@/components/sessions/DeleteSessionButton';

const ACTIVITY_TYPE_KEYS: Record<string, string> = {
    kit_arrangement: 'kitArrangement',
    supervision: 'supervision',
    other: 'other',
};

export default async function CoachSessionsPage({
    searchParams,
}: {
    searchParams: Promise<{ tab?: string }>;
}) {
    const supabase = await createClient();
    const locale = await getLocale();
    const t = await getTranslations('pages.sessions');
    const tc = await getTranslations('common');

    const { data: { user } } = await supabase.auth.getUser();
    if (!user) redirect('/login');

    const { tab } = await searchParams;
    const activeTab = tab === 'activities' ? 'activities' : 'sessions';

    // Sessions (course_id IS NOT NULL)
    const { data: sessions } = await supabase
        .from('sessions')
        .select(`
            id,
            session_date,
            start_time,
            end_time,
            computed_hours,
            applied_rate,
            subtotal,
            session_type,
            notes,
            courses (id, name),
            originally_scheduled_coach:profiles!sessions_originally_scheduled_coach_id_fkey (full_name)
        `)
        .eq('paid_coach_id', user.id)
        .not('course_id', 'is', null)
        .order('session_date', { ascending: false })
        .order('start_time', { ascending: false });

    // Activities (course_id IS NULL, activity_type IS NOT NULL)
    const { data: activities } = await supabase
        .from('sessions')
        .select('id, session_date, start_time, end_time, computed_hours, applied_rate, subtotal, activity_type, activity_description')
        .eq('paid_coach_id', user.id)
        .is('course_id', null)
        .not('activity_type', 'is', null)
        .order('session_date', { ascending: false })
        .order('start_time', { ascending: false });

    // Group by month helpers
    const groupByMonth = (rows: any[]) =>
        rows.reduce((acc: any, row: any) => {
            const month = row.session_date.substring(0, 7);
            if (!acc[month]) acc[month] = [];
            acc[month].push(row);
            return acc;
        }, {});

    const sessionsByMonth = groupByMonth(sessions || []);
    const activitiesByMonth = groupByMonth(activities || []);
    const currentMonth = new Date().toISOString().substring(0, 7);

    return (
        <div className="page-container">
            <div className="max-w-7xl mx-auto">
                {/* Header */}
                <div className="flex justify-between items-start flex-wrap gap-3 mb-6 md:mb-8">
                    <div>
                        <h1 className="text-2xl md:text-4xl font-bold text-white mb-2">{t('title')}</h1>
                        <p className="text-white/70">{t('subtitle')}</p>
                    </div>
                    {activeTab === 'sessions' ? (
                        <Link href={`/${locale}/coach/sessions/new`} className="btn-glossy">
                            + {t('logSession')}
                        </Link>
                    ) : (
                        <Link href={`/${locale}/coach/sessions/log-activity`} className="btn-glossy">
                            {t('logActivity')}
                        </Link>
                    )}
                </div>

                {/* Tabs */}
                <div className="flex gap-1 mb-6 bg-white/5 rounded-xl p-1 w-fit">
                    <Link
                        href={`/${locale}/coach/sessions?tab=sessions`}
                        className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${
                            activeTab === 'sessions'
                                ? 'bg-primary text-white shadow'
                                : 'text-white/60 hover:text-white'
                        }`}
                    >
                        {t('sessionsTab')}
                    </Link>
                    <Link
                        href={`/${locale}/coach/sessions?tab=activities`}
                        className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${
                            activeTab === 'activities'
                                ? 'bg-primary text-white shadow'
                                : 'text-white/60 hover:text-white'
                        }`}
                    >
                        {t('activitiesTab')}
                    </Link>
                </div>

                {/* Sessions Tab */}
                {activeTab === 'sessions' && (
                    <>
                        {Object.keys(sessionsByMonth).map((month) => {
                            const monthSessions = sessionsByMonth[month];
                            const isCurrentMonth = month === currentMonth;
                            const monthTotal = monthSessions.reduce((sum: number, s: any) => sum + Number(s.subtotal), 0);

                            return (
                                <GlassCard key={month} className="mb-6">
                                    <div className="flex justify-between items-start flex-wrap gap-2 mb-4">
                                        <h2 className="text-2xl font-semibold text-white">
                                            {new Date(month + '-01').toLocaleDateString('en-US', { year: 'numeric', month: 'long' })}
                                            {isCurrentMonth && <span className="ml-2 text-sm text-primary">({t('currentMonth')})</span>}
                                        </h2>
                                        <div className="text-right">
                                            <div className="text-sm text-white/70">Total</div>
                                            <div className="text-2xl font-bold text-secondary">{formatCurrency(monthTotal)}</div>
                                        </div>
                                    </div>

                                    <div className="overflow-x-auto">
                                        <table className="w-full">
                                            <thead>
                                                <tr className="border-b border-white/10">
                                                    <th className="text-left py-3 px-4 text-white/70">{tc('date')}</th>
                                                    <th className="text-left py-3 px-4 text-white/70">{tc('course')}</th>
                                                    <th className="text-left py-3 px-4 text-white/70">{t('time')}</th>
                                                    <th className="text-left py-3 px-4 text-white/70">{t('type')}</th>
                                                    <th className="text-left py-3 px-4 text-white/70">{t('hrs')}</th>
                                                    <th className="text-left py-3 px-4 text-white/70">{t('rate')}</th>
                                                    <th className="text-right py-3 px-4 text-white/70">{tc('amount')}</th>
                                                    <th className="text-right py-3 px-4 text-white/70">{tc('actions')}</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {monthSessions.map((session: any) => (
                                                    <tr key={session.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                                        <td className="py-3 px-4 text-white">{formatDate(session.session_date)}</td>
                                                        <td className="py-3 px-4 text-white">
                                                            {session.courses?.name ?? <span className="text-white/40 italic">Course unavailable</span>}
                                                            {session.originally_scheduled_coach && (
                                                                <div className="text-xs text-yellow-400">
                                                                    Replaced: {session.originally_scheduled_coach.full_name}
                                                                </div>
                                                            )}
                                                        </td>
                                                        <td className="py-3 px-4 text-white text-sm">{session.start_time} - {session.end_time}</td>
                                                        <td className="py-3 px-4">
                                                            <span className={`text-xs px-2 py-1 rounded ${session.session_type === 'online_session'
                                                                ? 'bg-blue-500/20 text-blue-300'
                                                                : 'bg-purple-500/20 text-purple-300'
                                                            }`}>
                                                                {session.session_type === 'online_session' ? tc('online') : tc('offline')}
                                                            </span>
                                                        </td>
                                                        <td className="py-3 px-4 text-white">{session.computed_hours}h</td>
                                                        <td className="py-3 px-4 text-white">{formatCurrency(session.applied_rate)}</td>
                                                        <td className="py-3 px-4 text-white text-right font-semibold">{formatCurrency(session.subtotal)}</td>
                                                        <td className="py-3 px-4 text-right">
                                                            {isCurrentMonth && (
                                                                <div className="flex justify-end gap-2">
                                                                    <Link
                                                                        href={`/${locale}/coach/sessions/edit/${session.id}`}
                                                                        className="text-primary hover:text-primary-light transition-colors text-sm"
                                                                    >
                                                                        {tc('edit')}
                                                                    </Link>
                                                                    <DeleteSessionButton sessionId={session.id} />
                                                                </div>
                                                            )}
                                                        </td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                                    </div>
                                </GlassCard>
                            );
                        })}

                        {(!sessions || sessions.length === 0) && (
                            <GlassCard>
                                <div className="text-center py-12">
                                    <p className="text-white/70 mb-4">{t('noSessions')}</p>
                                    <Link href={`/${locale}/coach/sessions/new`} className="btn-glossy inline-block">
                                        {t('logFirst')}
                                    </Link>
                                </div>
                            </GlassCard>
                        )}
                    </>
                )}

                {/* Activities Tab */}
                {activeTab === 'activities' && (
                    <>
                        {Object.keys(activitiesByMonth).map((month) => {
                            const monthActivities = activitiesByMonth[month];
                            const isCurrentMonth = month === currentMonth;
                            const monthTotal = monthActivities.reduce((sum: number, a: any) => sum + Number(a.subtotal), 0);

                            return (
                                <GlassCard key={month} className="mb-6">
                                    <div className="flex justify-between items-start flex-wrap gap-2 mb-4">
                                        <h2 className="text-2xl font-semibold text-white">
                                            {new Date(month + '-01').toLocaleDateString('en-US', { year: 'numeric', month: 'long' })}
                                            {isCurrentMonth && <span className="ml-2 text-sm text-primary">({t('currentMonth')})</span>}
                                        </h2>
                                        <div className="text-right">
                                            <div className="text-sm text-white/70">Total</div>
                                            <div className="text-2xl font-bold text-secondary">{formatCurrency(monthTotal)}</div>
                                        </div>
                                    </div>

                                    <div className="overflow-x-auto">
                                        <table className="w-full">
                                            <thead>
                                                <tr className="border-b border-white/10">
                                                    <th className="text-left py-3 px-4 text-white/70">{tc('date')}</th>
                                                    <th className="text-left py-3 px-4 text-white/70">{t('activityType')}</th>
                                                    <th className="text-left py-3 px-4 text-white/70">{t('time')}</th>
                                                    <th className="text-left py-3 px-4 text-white/70">{t('hrs')}</th>
                                                    <th className="text-left py-3 px-4 text-white/70">{t('rate')}</th>
                                                    <th className="text-right py-3 px-4 text-white/70">{tc('amount')}</th>
                                                    <th className="text-right py-3 px-4 text-white/70">{tc('actions')}</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {monthActivities.map((activity: any) => (
                                                    <tr key={activity.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                                        <td className="py-3 px-4 text-white">{formatDate(activity.session_date)}</td>
                                                        <td className="py-3 px-4">
                                                            <span className="text-xs px-2 py-1 rounded bg-emerald-500/20 text-emerald-300">
                                                                {t(ACTIVITY_TYPE_KEYS[activity.activity_type] as any)}
                                                            </span>
                                                            {activity.activity_description && (
                                                                <div className="text-xs text-white/50 mt-1 max-w-xs truncate">
                                                                    {activity.activity_description}
                                                                </div>
                                                            )}
                                                        </td>
                                                        <td className="py-3 px-4 text-white text-sm">{activity.start_time} - {activity.end_time}</td>
                                                        <td className="py-3 px-4 text-white">{activity.computed_hours}h</td>
                                                        <td className="py-3 px-4 text-white">{formatCurrency(activity.applied_rate)}</td>
                                                        <td className="py-3 px-4 text-white text-right font-semibold">{formatCurrency(activity.subtotal)}</td>
                                                        <td className="py-3 px-4 text-right">
                                                            {isCurrentMonth && (
                                                                <DeleteSessionButton sessionId={activity.id} />
                                                            )}
                                                        </td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                                    </div>
                                </GlassCard>
                            );
                        })}

                        {(!activities || activities.length === 0) && (
                            <GlassCard>
                                <div className="text-center py-12">
                                    <p className="text-white/70 mb-4">{t('noActivities')}</p>
                                    <Link href={`/${locale}/coach/sessions/log-activity`} className="btn-glossy inline-block">
                                        {t('logFirstActivity')}
                                    </Link>
                                </div>
                            </GlassCard>
                        )}
                    </>
                )}
            </div>
        </div>
    );
}
